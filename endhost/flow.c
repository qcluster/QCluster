...
void PIAS_Print_Table(struct PIAS_Flow_Table* ft)
{
	...
	if (unlikely(!ft))
	{
		printk(KERN_EMERG "PIAS_Print_Table: NULL pointer\n");
		return;
	}

	printk(KERN_EMERG "PIAS: current flow table\n");
	for (i = 0; i < PIAS_HASH_RANGE; i++)
	{
		if (ft->flow_lists[i].len > 0)
		{
        	printk(KERN_EMERG "PIAS: flowlist %d\n", i);
			PIAS_Print_List(&(ft->flow_lists[i]));
		}
	}
	printk(KERN_EMERG "PIAS: there are %d flows in total\n", atomic_read(&(ft->size)));
}
...
bool PIAS_Init_Info(struct PIAS_Flow_Info* info)
{
	...
	if (likely(info))
	{
		...
		info->timeouts = 0;
		info->messages = 0;
		memset(info->queue_dist, 0, sizeof(info->queue_dist));
		return true;
	}
	else
	{
...
bool PIAS_Insert_List(struct PIAS_Flow_List* fl, struct PIAS_Flow* f, int flags)
{
	...
	if (buf = PIAS_Search_List(fl, f))
	{
		// TODO: comment this line
    	printk(KERN_INFO "PIAS_Insert_List: equal flow\n");
    	
    	// Jie: If exist the same flow, clear it
    	buf->info = f->info;

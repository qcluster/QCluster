...
for kind in range(len(workloadName)):
    for prio_num_ in prio_num_arr:
        for i in range(len(load_arr)):
            ...
            directory_name = 'pias_%s_%d_%f_%.2d%.2d' % (
                    workloadName[kind], int(load_arr[i]*100), interval_message[kind]*10000, 
            ...
            cmd = ns_path+' '+sim_script+' '\
                ...
                +str(prio_num_)+' '\
                +str(ECN_scheme_)+' '\
                +str(pias_thresh_0[kind][i])+' '\
                +str(pias_thresh_1[kind][i])+' '\
                +str(pias_thresh_2[kind][i])+' '\
                +str(pias_thresh_3[kind][i])+' '\
                +str(pias_thresh_4[kind][i])+' '\
                +str(pias_thresh_5[kind][i])+' '\
                +str(pias_thresh_6[kind][i])+' '\
                +str(topology_spt)+' '\
                +str(topology_tors)+' '\
                +str(topology_spines)+' '\
                +str(topology_x)+' '\

...
#		    continue;
		...
		if byte_size<=100*1000:
			all_thp += byte_size
			ddl = 80*math.pow(2.0, math.ceil(byte_size/1460.0)/16.0)
			if int(ddl) >= result*1000:
			    thp += byte_size	
			lesshundredKB.append(result)
		else:
			other.append(result)
	...
	if args.all:
		...
		print "Overall average FCT is:\n"+str(avg(flows))
	        print "Average FCT for "+str(len(other))+" flows in (>100KB)\n"+str(avg(other))

...
FullTcpAgent::delay_bind_init_all()
{
    ...
    delay_bind_init_one("enable_known_");
    delay_bind_init_one("thres_message_");	
    delay_bind_init_one("enable_ddl_");
...
FullTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
    ...
    if (delay_bind_bool(varName, localName, "enable_known_", &enable_known_, tracer)) return TCL_OK;
    if (delay_bind(varName, localName, "thres_message_", &thres_message_, tracer)) return TCL_OK;
    if (delay_bind_bool(varName, localName, "enable_ddl_", &enable_ddl_, tracer)) return TCL_OK;
...
//	//ssthresh_ = cwnd_;
	...
	case TCPS_LISTEN:
		...
		if(enable_ddl_ && nb <= 100000)
		    deadline = int(80*pow(2,ceil(nb/1460.0)/16.0));
		else
		    deadline = 0;
		break;
...
FullTcpAgent::sendpacket(int seqno, int ackno, int pflags, int datalen, int reason, Packet *p)
{
	...
	/* actual size is data length plus header length */
        ...
        if (datalen <= 0) {
                ...
                ++ndatapack_;
                ndatabytes_ += datalen;
		if(now() - last_send_time_ >= thres_message_ && enable_pias_)
		    pias_startseq_ = seqno - datalen;
...
//				iph->prio() = iph->prio() / 40 * 1000 + set_prio(seqno, curseq_) / 1460;
    ...
    if(enable_pias_)
    {
        if(datalen>0)
        {
            iph->prio()=piasPrio(seqno-pias_startseq_);
            if(pias_debug_)
    ...
    if(enable_known_){
	if(enable_ddl_ && deadline > 0){
	   int tleft = deadline - int((now() - start_time) * 1e6);
	   if(tleft <= 0){
		iph->prio() = 1 << 30;
	   }
	   else{
		iph->prio() = tleft * 15 / max(1,ceil((curseq_ - seqno)/1460.0));
	   }
	}
	else{
	    if(datalen>0)
            {
                iph->prio() = curseq_ - seqno + 1460 * (enable_ddl_);
            }
            else
            {
                iph->prio() = 0;
	...
	*/
	...
	if (!nopredict_ && predict_ok(pkt)) {
		...
		if (datalen == 0) {
			...
			//	flow_remaining_ -= (rcv_nxt_ - rcv_nxt_old_);
			}

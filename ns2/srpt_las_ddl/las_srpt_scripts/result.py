...
if args.input:
	fp = open(args.input)
#	fout1 = open("trash1.dat", "a")
#	fout2 = open("trash2.dat", "a")
#	mylist = str(args.input).split("_")
#	print (mylist)
...
#		    continue;
		...
		if byte_size<1024:
			lessKB.append(result)
		elif byte_size<10*1024:
			lesstenKB.append(result)
		else:
			other.append(result)
		...
		if timeouts_num>0:
                        #If the flow is a short flow
			if byte_size<=1024:
				lessKB_to += timeouts_num
                        elif byte_size<=10*1024:
#				print byte_size
                                lesstenKB_to += timeouts_num
                        #If the flow is a large flow
			else:
				other_to += timeouts_num
		
	fp.close()
#	fout1.write(mylist[1]+","+mylist[2]+","+str(avg(flows))+"\n")
#	fout2.write(mylist[1]+","+mylist[2]+","+str(avg(lesstenKB))+"\n")
#	fout1.close()
#	fout2.close()
	if args.all:
		...
		print "Average FCT for "+str(len(lessKB))+" flows in (0,1KB)\n"+str(avg(lessKB))
                print "Timeout " + str(lessKB_to)
                print "99th percentile FCT for "+str(len(lessKB))+" flows in (0,1KB)\n"+str(max(lessKB))
                print "95th small flow:\n" + str(per95(lessKB))
                print "90th small flow:\n" + str(per90(lessKB))
		print "Average FCT for "+str(len(lesstenKB))+" flows in (1KB,10KB)\n"+str(avg(lesstenKB))
		print "Timeout " + str(lesstenKB_to)
		print "99th percentile FCT for "+str(len(lesstenKB))+" flows in (0,1KB)\n"+str(max(lesstenKB))
		print "Average FCT for "+str(len(other))+" flows in (>10KB)\n"+str(avg(other))
                print "Timeout " + str(other_to)
